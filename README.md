...
### Levar para o disparador

`[ BAIXAR P/ DISPARADOR ]` gera o CSV no formato que o Disparador de E-mail
importa direto, sem mapear campo nenhum:

```
email,nome,cidade,segmento
```

`email` e `nome` são os nomes que o parser reconhece; `cidade` e `segmento` viram
as variáveis `{{cidade}}` e `{{segmento}}` no corpo da mensagem — dá para escrever
*"vi que vocês trabalham com {{segmento}} aí em {{cidade}}"*.

Detalhes que o botão resolve sozinho:

- **Divide em blocos de 50 mil linhas.** A importação recusa acima de 200 mil por
  arquivo e a base passa disso. Saem como `..._parte1de7.csv` e afins — o
  navegador pede confirmação de "vários downloads" na primeira vez.
- **Descarta e-mail malformado** com a mesma regra do disparador (foram 732 na
  base completa), para não virar bounce.
- **Nome vazio no lugar de `NÃO DETECTADO`**, para o fallback `{{nome|amigo}}`
  assumir em vez de sair *"Olá NÃO DETECTADO"*.

Respeita o filtro da tela: filtrar por cidade ou segmento antes de baixar é a
forma natural de fatiar a lista.

**Sobre `{{primeiro_nome}}`:** ele pega a primeira palavra do nome, o que funciona
para MEI (nome de pessoa) mas trunca razão social — `MW GRAOS LOGISTICA` vira
*"MW"*. Dos e-mails válidos, 40% são MEI e 60% são empresa. Para usar primeiro
nome, marque **só MEI** antes de exportar; para o resto, use `{{nome}}`.
